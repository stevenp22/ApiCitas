                ...
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body table-responsive">
                        <table id="tabla" class="table table-striped table-hover">
                            ...
                            <tbody>
                                <?php
                                    include "../../models/bd/conexionBD.php";
        
                                    $sql = "SELECT U.id, U.nombre_c, U.usuario, U.correo, R.rol, I.institucion nombre_institucion, M.municipio, Date(U.fecha_creacion) fecha_creacion, U.estado
                                    FROM usuarios U 
                                    INNER JOIN roles R ON U.rol = R.id
                                    INNER JOIN instituciones I ON U.institucion = I.id
                                    INNER JOIN municipios M ON I.municipio = M.id
                                    ORDER BY U.rol ASC";
                                    ...
                                    if ($result->num_rows > 0) {
                                        ...
                                        while($row = $result->fetch_assoc()) {
                                            ...
                                            <td>". $row["estado"]. "</td>";
                                            if($row["estado"] == 'Activo' && $row['rol'] == 'Administrador'){
                                                echo "<td></td>";
                                            }else if($row["estado"] == 'Activo' && ($row['rol'] == 'Editor' || $row['rol'] == 'Reportes')){
                                                echo "
                                                <td>
